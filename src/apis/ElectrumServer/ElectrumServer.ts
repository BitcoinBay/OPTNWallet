...
export default function ElectrumService() {
    async function electrumConnect(server: string = testServer): Promise<ElectrumClient> {
        const electrum = new ElectrumClient('OPTNWallet', '1.4.1', server, ElectrumTransport.WSS.Port, ElectrumTransport.WSS.Scheme);
        await electrum.connect();
        return electrum;
    }

    async function electrumDisconnect(electrum: ElectrumClient): Promise<boolean> {
    ...
    async function getUTXOS(address: string): Promise<any> {
        const electrum = await electrumConnect();
        try {
            const UTXOs = await electrum.request("blockchain.address.listunspent", address);
    ...
    async function broadcastTransaction(tx_hex: string) {
        const electrum = await electrumConnect();
        try {
            const tx_hash = await electrum.request("blockchain.transaction.broadcast", tx_hex);
            return tx_hash;
        } finally {
            await electrumDisconnect(electrum);
        }
