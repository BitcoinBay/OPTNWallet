...
const Transaction = () => {
    ...
    useEffect(() => {
        const fetchWalletId = async () => {
            ...
            const activeWalletId = 14; // Replace this with the actual logic to fetch the active wallet ID
    ...
    useEffect(() => {
        const fetchAddressesAndUTXOs = async (walletId: number) => {
            ...
            console.log("addressesStatement:", addressesStatement)
            ...
            while (utxosStatement.step()) {
                ...
                fetchedUTXOs.push({
                    ...
                    privateKey: new Uint8Array() // This should be replaced with the actual private key
                });
            }
            utxosStatement.free();
    ...
    const toggleUtxoSelection = (utxo: UTXO) => {
        if (selectedUtxos.includes(utxo)) {
            setSelectedUtxos(selectedUtxos.filter(selectedUtxo => selectedUtxo.id !== utxo.id));
        } else {
    ...
    const addOutput = () => {
        if (recipientAddress && transferAmount) {
            setOutputs([...outputs, { recipientAddress, amount: Number(transferAmount) }]);
            setRecipientAddress('');
            setTransferAmount('');
        }
    };
    ...
    const buildTransaction = async () => {
        ...
        const transaction = await txBuilder.buildTransaction(selectedUtxos, outputs);
    ...
    return (
        <div>
            ...
            <div>
                ...
                {utxos.filter(utxo => selectedAddresses.includes(utxo.address)).map((utxo, index) => (
                    <div key={index}>
                        <input 
                            type="checkbox" 
                        ...
                        />
                        <span>{`Address: ${utxo.address}, Amount: ${utxo.amount} ${utxo.tx_hash} ${utxo.tx_pos}`}</span>
                    </div>
                ))}
            </div>
            <div>
